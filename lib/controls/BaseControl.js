...
class BaseControl extends EventEmitter {
    ...
    constructor (options={}) {
        ...
        this.controlName = options.controlName || 'BaseControl';
    ...
    destroy () {
        this.emit(ControlEvents.DESTROY);
    }

    emitChangeEvent() {
        this.emit(ControlEvents.VALUE_CHANGE, {
            value: this.value,
    ...
    get percent () {
        ...
        return utils.clamp((percent.toFixed(8)/1), 0, 1);
    }
    ...
    get value () {
        ...
        return utils.clamp((transformed.toFixed(8)/1), this.min, this.max);
    }
