...
import { ControlTypes, ControlEvents } from '../constants';
        ...
        if(this.__isValidControlType(controlType)) {
            if (this.controlType !== controlType) {
                ...
                if(this.__control != null) {
                    this.__control.removeListener(ControlEvents.VALUE_CHANGE, this.__controlListener);
                }

                this.model.controlType = controlType;
                this.emit(ControlEvents.CONTROL_TYPE_CHANGE, { controlType: controlType });
                this.__control = this.__controls[controlType];
                this.__control.on(ControlEvents.VALUE_CHANGE, this.__controlListener);
            }
        } else {
